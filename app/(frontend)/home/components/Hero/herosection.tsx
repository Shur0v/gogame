  ...
  useEffect(() => {
    if (heroTextRef.current) {
      ...
      gsap.to(heroTextRef.current.children, {
        ...
        stagger: 0.2,
        ease: "power3.out",
        delay: 3.6 // Increased delay from 0.5 to 1.5 seconds
      });
    }
  }, []);

  return (
    <div className="w-full h-[720px] relative flex-shrink-0">
      <div 
      ...
      <div className="relative w-[1200px] h-full mx-auto p-6 flex flex-col justify-end items-center gap-2.5">
        <div className="w-full flex flex-col justify-start items-start gap-8">
          <div ref={heroTextRef} className="max-w-[1041px] flex flex-col justify-start items-start gap-6">
            <h1 className="text-white text-7xl font-extrabold font-['Inter'] leading-[86.40px]">
              Are you ready to experience sports like never before?
            </h1>
            <p className="text-white text-lg font-normal font-['Poppins'] leading-loose">
              Let your passion for soccer or basketball take you to an unexpected place. The destination is a surprise.
            </p>
          </div>
          <div className="w-full p-3 bg-white rounded-lg backdrop-blur-[5px] flex flex-col justify-start items-start gap-3" ref={dropdownRef}>
            <div className="bg-gray-50 rounded-lg inline-flex justify-start items-center">
              {(['Football', 'Basketball', 'Both'] as const).map((sport, index, array) => (
                <button
                  ...
                  className={`w-36 h-11 px-3.5 py-1.5 flex justify-center items-center gap-2.5 ${
                    selectedSport === sport
            ...
            <div className="w-full p-3 bg-gray-50 rounded-xl inline-flex justify-start items-end gap-3">
              {/* Pack Type Dropdown */}
              <div className="flex-1 inline-flex flex-col justify-center items-start gap-2 relative">
                ...
                <div 
                  ...
                  className="cursor-pointer w-full h-11 px-3.5 py-1.5 bg-white rounded outline outline-1 outline-offset-[-1px] outline-neutral-300 inline-flex justify-between items-center"
                >
                  <span className="text-zinc-950 text-sm font-normal font-['Poppins'] leading-relaxed">
                    {selectedPack.name} - {selectedPack.price}
                  </span>
                  <IoChevronDown className={`w-5 h-5 text-gray-500 transition-transform duration-200 ${openDropdown === 'pack' ? 'rotate-180' : ''}`} />
                </div>
                {openDropdown === 'pack' && (
              ...
              <div className="flex-1 inline-flex flex-col justify-center items-start gap-2 relative">
                ...
                >
                  ...
                  <IoChevronDown className={`w-5 h-5 text-gray-500 transition-transform duration-200 ${openDropdown === 'city' ? 'rotate-180' : ''}`} />
                </div>
                {openDropdown === 'city' && (
                  <div className="absolute top-[100%] left-0 right-0 mt-1 bg-white rounded-lg shadow-lg z-[100] py-1 border border-gray-200">
              ...
              <div className="flex-1 inline-flex flex-col justify-center items-start gap-2 relative">
                ...
                >
                  ...
                  <IoChevronDown className={`w-5 h-5 text-gray-500 transition-transform duration-200 ${openDropdown === 'people' ? 'rotate-180' : ''}`} />
                </div>
                {openDropdown === 'people' && (
                  <div className="absolute top-[100%] left-0 right-0 mt-1 bg-white rounded-lg shadow-lg z-[100] py-1 border border-gray-200">
              ...
              <button 
                ...
                className="w-44 h-11 px-3.5 py-1.5 bg-lime-500 rounded backdrop-blur-[5px] flex justify-center items-center gap-2.5 hover:bg-lime-600 transition-colors"
              >
